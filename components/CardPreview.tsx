        ...
        return (
          <div className="relative w-full h-full p-8 flex flex-col justify-between bg-white text-slate-900 overflow-hidden">
            ...
            <div style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'left top' }}>
              <h2 className="text-3xl font-light tracking-tight">{data.name}</h2>
              <p className="text-sm font-medium tracking-widest uppercase opacity-60 mt-1" style={{ color: style.primaryColor }}>{data.position}</p>
            </div>
            <div className="flex flex-col gap-1 uppercase tracking-wider text-slate-400 font-medium" style={{ fontSize: `${s(10)}px` }}>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1"><Mail size={s(10)} /> {data.email}</span>
                <span className="w-1 h-1 rounded-full bg-slate-200" />
                <span className="flex items-center gap-1"><Phone size={s(10)} /> {data.contact}</span>
              </div>
              <div className="flex items-center gap-4">
                <span className="flex items-center gap-1"><Github size={s(10)} /> {data.github}</span>
                <span className="w-1 h-1 rounded-full bg-slate-200" />
                <span className="flex items-center gap-1"><Globe size={s(10)} /> {data.blog}</span>
              </div>
            </div>
        ...
        return (
          <div className="w-full h-full p-0 flex flex-col bg-slate-50 relative overflow-hidden font-montserrat">
            ...
            <div className="flex-1 p-8 flex flex-col justify-center">
               <h2 className="font-black mb-1 leading-none italic" style={{ color: style.primaryColor, fontSize: `${s(40)}px` }}>{data.name}</h2>
               <p className="font-bold tracking-tighter text-slate-400 mb-6" style={{ fontSize: `${s(14)}px` }}>{data.position}</p>
               <div className="max-w-[80%]">
                 <p className="leading-relaxed text-slate-600 font-semibold border-l-4 pl-3" style={{ borderColor: style.accentColor, fontSize: `${s(12)}px` }}>{data.goal}</p>
               </div>
            </div>

            <div className="bg-slate-900 p-6 flex justify-between items-center text-white">
              <div className="space-y-1">
                <p className="flex items-center gap-2" style={{ fontSize: `${s(10)}px` }}><Phone size={s(10)} className="text-slate-400"/> {data.contact}</p>
                <p className="flex items-center gap-2" style={{ fontSize: `${s(10)}px` }}><Mail size={s(10)} className="text-slate-400"/> {data.email}</p>
              </div>
            ...
            <div className="col-span-11 p-10 flex flex-col justify-between">
              <div className="flex justify-between items-start">
                <div style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'left top' }}>
                  ...
                  <p className="text-sm font-medium tracking-wide text-slate-500">{data.position}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 mt-6">
                <InfoItem icon={<Phone size={s(14)} />} text={data.contact} scale={style.contentScale} />
                <InfoItem icon={<Github size={s(14)} />} text={data.github} scale={style.contentScale} />
                <InfoItem icon={<Mail size={s(14)} />} text={data.email} scale={style.contentScale} />
                <InfoItem icon={<Globe size={s(14)} />} text={data.blog} scale={style.contentScale} />
              </div>
      ...
      case 'dark':
        return (
          <div className="w-full h-full p-10 flex flex-col justify-between bg-slate-950 text-white overflow-hidden relative">
            ...
            <div style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'left top' }}>
              <p className="text-[10px] font-bold tracking-[0.3em] text-slate-500 mb-1 uppercase">{data.tagline}</p>
              <h2 className="text-4xl font-bold tracking-tight mb-1">{data.name}</h2>
              <div className="flex items-center gap-2">
                <div className="w-4 h-[2px]" style={{ backgroundColor: style.primaryColor }} />
                <p className="text-sm font-medium text-slate-400">{data.position}</p>
              </div>
            </div>

            <div className="flex justify-between items-end">
              <div className="space-y-1.5" style={{ fontSize: `${s(12)}px` }}>
                <p className="font-mono text-slate-300 flex items-center gap-2"><Mail size={s(12)} /> {data.email}</p>
                <p className="font-mono text-slate-300 flex items-center gap-2"><Phone size={s(12)} /> {data.contact}</p>
              </div>
              <div className="text-right space-y-0.5" style={{ fontSize: `${s(12)}px` }}>
                <p className="font-bold flex items-center justify-end gap-2" style={{ color: style.accentColor }}><Github size={s(12)} /> {data.github}</p>
        ...
        return (
          <div className="w-full h-full p-8 flex flex-col justify-between relative overflow-hidden bg-white/10 backdrop-blur-xl border border-white/20">
            ...
            <div className="space-y-1" style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'left top' }}>
              <h2 className="text-3xl font-montserrat font-extrabold text-slate-800 drop-shadow-sm">{data.name}</h2>
            ...
            <div className="flex justify-between items-center font-bold text-slate-600" style={{ fontSize: `${s(11)}px` }}>
              <div className="flex items-center gap-3">
                <span className="flex items-center gap-1"><Mail size={s(11)} /> {data.email.split('@')[0]}</span>
                <span className="w-[1px] h-3 bg-slate-300/50" />
                <span className="flex items-center gap-1"><Phone size={s(11)} /> {data.contact}</span>
              </div>
        ...
        return (
          <div className="w-full h-full p-0 flex flex-col bg-white overflow-hidden shadow-2xl relative">
            ...
            <div className="flex-1 p-8 flex flex-col justify-between">
              <div className="flex justify-between items-start">
                <div style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'left top' }}>
                  <h2 className="text-4xl font-playfair text-slate-900 mb-0.5">{data.name}</h2>
                  <p className="text-xs tracking-[0.2em] font-bold text-slate-400 uppercase">{data.position}</p>
                </div>
                <div className="text-right" style={{ transform: `scale(${style.contentScale})`, transformOrigin: 'right top' }}>
                  ...
                  <p className="text-xs font-bold text-slate-800" style={{ color: style.accentColor }}>{data.tagline}</p>
                </div>
              </div>

              <div className="max-w-[85%] mt-4">
                 <p className="leading-relaxed text-slate-600 font-medium" style={{ fontSize: `${s(11)}px` }}>{data.goal}</p>
              </div>

              <div className="flex items-center justify-between border-t border-slate-100 pt-6 mt-4">
                <div className="space-y-1">
                  <p className="font-bold flex items-center gap-2 text-slate-500 uppercase" style={{ fontSize: `${s(10)}px` }}><Phone size={s(12)} style={{ color: style.primaryColor }} /> {data.contact}</p>
                  <p className="font-bold flex items-center gap-2 text-slate-500 uppercase" style={{ fontSize: `${s(10)}px` }}><Mail size={s(12)} style={{ color: style.primaryColor }} /> {data.email}</p>
                </div>
                <div className="space-y-1 text-right">
                  <p className="font-mono font-medium text-slate-500 flex items-center justify-end gap-2" style={{ fontSize: `${s(10)}px` }}><Github size={s(12)} /> {data.github}</p>
                  <p className="font-mono font-medium text-slate-500" style={{ fontSize: `${s(10)}px` }}>{data.blog}</p>
                </div>
              </div>
            </div>
            {renderQR()}
