      ...
      <div className="absolute inset-0 p-12 flex flex-col justify-between">
        <div className="flex justify-between items-start">
          <div className="space-y-4">
            <div className="border-l-4 pl-6 py-2" style={{ borderColor: style.primaryColor }}>
            ...
            {renderDraggableField('role', <p className="text-[10px] text-white px-2 py-0.5 inline-block" style={{ backgroundColor: style.primaryColor }}>{displayData.position}</p>)}
          </div>
          <Target style={{ color: style.primaryColor, opacity: 0.2 }} size={48} />
        </div>

        <div className="grid grid-cols-2 gap-4 text-[10px] bg-white/80 backdrop-blur-sm p-4 border border-[#dee2e6]">
          <div className="space-y-1">
            {displayData.email && renderDraggableField('email', <div><span style={{ color: style.primaryColor }}>EMAIL:</span> {displayData.email}</div>)}
            {displayData.contact && renderDraggableField('contact', <div><span style={{ color: style.primaryColor }}>TEL:</span> {displayData.contact}</div>)}
          </div>
          <div className="space-y-1 border-l border-[#dee2e6] pl-4">
            {displayData.github && renderDraggableField('github', <div><span style={{ color: style.primaryColor }}>GITHUB:</span> {displayData.github}</div>)}
            {displayData.location && renderDraggableField('location', <div><span style={{ color: style.primaryColor }}>LOCATION:</span> {displayData.location}</div>)}
          </div>
        </div>
      </div>
