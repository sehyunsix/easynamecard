...
export const NeuralNode: ThemeComponent = ({ displayData, style, renderDraggableField, renderQRCodeElement, s, isBack }) => {
  return (
    <div className="w-full h-full bg-slate-950 text-white font-sans relative overflow-hidden">
      ...
      <svg className="absolute inset-0 w-full h-full opacity-20 pointer-events-none" viewBox="0 0 400 228">
      ...
      <div className="absolute inset-0 p-12 flex flex-col justify-between">
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            ...
            {renderDraggableField('position', <p className="text-[10px] font-bold text-blue-400 tracking-[0.4em] uppercase">{displayData.position}</p>)}
          </div>
          <Network className="text-blue-500 opacity-50" size={40} />
        </div>

        <div className="flex justify-between items-end">
          <div className="space-y-2 text-[10px] font-mono tracking-tight text-slate-400 border-l border-blue-500/50 pl-6">
            {displayData.email && renderDraggableField('email', <div className="flex items-center gap-2"><span>[E]</span> {displayData.email}</div>)}
            {displayData.github && renderDraggableField('github', <div className="flex items-center gap-2"><span>[G]</span> {displayData.github}</div>)}
            {displayData.location && renderDraggableField('location', <div className="flex items-center gap-2"><span>[L]</span> {displayData.location}</div>)}
          </div>
